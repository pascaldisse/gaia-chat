...
function AppContent() {
  ...
  const { provider, selectedModel, keyStatus, getDefaultModel } = useProvider();
  const [currentChat, setCurrentChat] = useState([]);
  const [model, setModel] = useState(getDefaultModel());
  const [systemPrompt] = useState('You are a helpful assistant.');
  ...
  return (
    <div className="app">
      <div
        className={`provider-status ${provider.apiKey ? 'ready' : 'missing'}`}
